...
目的: 機械が書く 1 枚（`docs/SCOPE_PROGRESS.md`）を朝の儀式の前に作り直す。手で書かない。正本は `08プロジェクト/<プロジェクト>/status.md`・`00廷議/*.md`・scheduler・`09アーカイブ/案件/` の墓標。

`docs/SCOPE_PROGRESS.md` の型（8 節固定・順固定・空の節は「（なし）」1 行）の正本は `scripts/build_scope_progress.py` の docstring。**この紙も生成器もその型に従う。**

## やること（この順）
0. **`run_start`**（`seki: "scheduler"`・`bin: "SCOPE_PROGRESSの生成"`）＝走ったことの正本。撃ち方は `_実行を帳簿へ書く.md`
1. `git -C {棚の置き場} pull --rebase --autostash`
2. **稼働中の席を取る**: MCP `list_monsters` の結果を JSON で `11一時ファイル/monsters-YYYY-MM-DD.json` に落とす。取れなければ次へ進む（§6 の稼働中が「（未取得）」になるだけ）
...
4. **実行を帳簿へ**（`_実行を帳簿へ書く.md`）: 段 0 の id に `run_end`（`kekka`＝成功／失敗／手が要った）。中身の 1 行は**スクリプトの stderr の 1 行をそのまま**。**帳簿に書かずに終わったら失敗**
5. `docs/SCOPE_PROGRESS.md` を **`git add -- <path>`** → commit「生成: SCOPE_PROGRESS YYYY-MM-DD」→ push（`git add -A` は禁止・規矩 X-1）
...
| 5 Routine | **止**・手順書なし・**実行なし**（帳簿に走った記録が無い） | scheduler と `docs/定期便-手順/<便>.md`。実行は `run_start`／`run_end` | 便の owner |
| 7 完了 | — | `09アーカイブ/案件/` の墓標 | 閉じた席 |
...
## 詰まったら
- スクリプトが落ちた → 中身の 1 行に `落ちた: <script>（1 行目のエラー）` を書き、`run_end` を `失敗` で閉じ、記録 MCP `kiroku` の `ledger_append` を 1 回（種別＝躓き。規矩 §4）
