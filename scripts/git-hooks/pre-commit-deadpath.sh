...
# 記録（docs/_Operations-Log.md・docs/定期便-ログ/・08プロジェクト/・09アーカイブ/・.claude/記録/）は
...
# 06エージェント資産/プロンプト資産/ も検査しない: BlueLamp の DB が正本の日次写しで、⛔ 人が本文を直さない
...
git -c core.quotepath=false diff --cached --name-only --diff-filter=ACM -- '*.md' \
 | grep -vE '^(docs/_Operations-Log\.md|docs/SCOPE_PROGRESS\.md|docs/定期便-ログ/|08プロジェクト/|09アーカイブ/|07学習資産/教材ライブラリ/|06エージェント資産/プロンプト資産/|\.claude/記録/)' \
 | while read -r f; do
